...
/* 	$.validator.addMethod('fileCheck', function(value, element){
	...
	$('form[name=bannerFm]').validate({
		...
		submitHandler: function(form) {
			...
			var callback = function(data) {
				...
				$("form[name=searchFm]")
					.attr("action","<c:out value='/banner/innoBannerList.do' />")
					.submit();
			}
	...
	$('#btnDelete').on('click', function(){
		if (!confirm(getMessage('confirm.delete'))) {
 		...
 		var url = '<c:url value="/banner/deleteInnoBanner.json"/>';
		var submitData = {
				bnnrIdx : '<c:out value="${bannerVO.bnnrIdx }"/>'
		}
		var callback = function(data) {
			alert(getMessage('msg.deleted'));
			location.href='<c:url value="/banner/innoBannerList.do" />';
		}
		ajaxSubmit(url, submitData, callback, null);
	});
	...
	$(document).ready(function(){
		...
		if("<c:out value='${bannerVO.bnnrIdx }' />" != ""){
			...
			$("input[name=link]").val('<c:out value="${bannerVO.link }"/>');
...
function fn_plfCheck(valueA, valueB, plf){
	...
	if(plf == '모바일'){
		$("#imgGuide").text('( 890*350 /20MB (협의중) )');
	}else{
		$("#imgGuide").text('( 890*350 /20MB 이하 )');
	}
