...
<c:forEach items="${woodparkClassScheduleMVOList}" var="item">
	$('select[name=wdpkClsIdx][data-clnd-dt="<c:out value="${item.clndDt}"/>"][data-srt-tm="<c:out value="${item.srtTm}"/>"][data-end-tm="<c:out value="${item.endTm}"/>"]')
	.find('option[value=<c:out value="${item.wdpkClsIdx}"/>]')
	.prop('selected', true)
	.end()
	.after('<br/>[<c:out value="${item.psblMaxPsct - rsvtCnt}"/>석 남음]');
</c:forEach>

	$('select[name=wdpkClsIdx]').each(function(){
		var clndDt = $(this).attr('data-clnd-dt');
		var srtTm = $(this).attr('data-srt-tm');
		var endTm = $(this).attr('data-end-tm');
		var tx = $(this).find('option:selected').text();

		if ($(this).val()) {
			$(this).parents('li:first').attr({
				'data-clnd-dt': clndDt,
				'data-srt-tm': srtTm,
				'data-end-tm': endTm
			});
...
jQuery(function($){
	...
	$('form[name=form1]').on('change', 'select[name=year], select[name=month]', function(){
		$('form[name=form]')
		.attr('action', '<c:url value="/reserve/facilities/woodpark_reserve_calendar.do"/>')
	...
	// 예약목록이동
	$('form[name=form1]')
	.on('click', 'li[data-clnd-dt]', function(){
		$('form[name=form]')
		.find('input:not([name=fctMstIdx])')
		.remove();

		$('form[name=form]')
		.attr('action', '<c:url value="/reserve/facilities/woodpark_reserve_list.do"/>')
		.append('<input type="hidden" name="clndDt" value="' + $(this).attr('data-clnd-dt') + '"/>')
		.append('<input type="hidden" name="srtTm" value="' + $(this).attr('data-srt-tm') + '"/>')
		.append('<input type="hidden" name="endTm" value="' + $(this).attr('data-end-tm') + '"/>')
		.submit();
...
<div id='samples' class='hidden'>
	<ul>
		...
		<li id='classScheduleElement'>
			<select name='wdpkClsIdx' DISABLED>
