...
jQuery(function($){
	...
	$('form[name=mediaFm]').validate({
		...
		submitHandler: function(form) {
			var url = '<c:url value="/information/media/insertMedia.json"/>';
			var submitData = $(form).serializeFormJSON();
				submitData.useYn = $("input[name=useYn]:checked").val();

			if("<c:out value='${mediaVO.mdaIdx}' />" != ""){
				url = '<c:url value="/information/media/updateMedia.json"/>';
				submitData.mdaIdx = "<c:out value='${mediaVO.mdaIdx}' />";
			}

			var callback = function(data) {
				alert(getMessage('msg.save.success'));
				location.href="/information/media/mediaList.do";
			}
	...
	$('#btnConfirm').on('click', function(){
		...
		$("form[name=mediaFm]").submit();
	});

	//삭제
	$('#btnDelete').on('click', function(){
		var url = '<c:url value="/information/media/deleteMedia.json"/>';
		var submitData = {
				mdaIdx : '<c:out value="${mediaVO.mdaIdx }" />'
		}

		var callback = function(data) {
			alert(getMessage('msg.deleted'));
			location.href='<c:url value="/information/media/mediaList.do" />';
		}
	...
	$(document).ready(function(){
		var $form = $("form[name=mediaFm]");
		
		<c:choose>
			<c:when test="${mediaVO.useYn eq 'Y'}">
				$form.find('#useYnOn').prop('checked', true);
			</c:when>
			<c:when test="${mediaVO.useYn eq 'N'}">
				$form.find('#useYnOff').prop('checked', true);
			</c:when>
		</c:choose>
		console.log("${mediaVO.pcYn eq 'Y'}");
		console.log("${mediaVO.mblYn eq 'Y'}");
		
		<c:if test="${mediaVO.pcYn eq 'Y'}" >
			$form.find('#pcYn').attr("checked",true);
		</c:if>

		<c:if test="${mediaVO.mblYn eq 'Y'}" >
			$form.find('#mblYn').attr("checked",true);
		</c:if>

 		$form.find('input[name=titl]').val('<c:out value="${mediaVO.titl }" />');
 		$form.find('input[name=writer]').val('<c:out value="${mediaVO.writer }" />');
 		$form.find('input[name=url]').val('<c:out value="${mediaVO.url }" />');
...
<div class="subcon_area">
	<form:form commandName="mediaVO" name="mediaFm">
	<div class="basicTbl02">
		<table>
			...
			<tr>
				<th>번호</th>
				<td><c:out value="${mediaVO.mdaIdx }" /></td>
				<th>등록일시</th>
				<td><c:out value="${mediaVO.creDttm }" /></td>
				<th>등록자</th>
				<td><c:out value="${creMngrMbrNm },${creMngrMbrDpt }" /> </td>
			</tr>
			<tr>
				<th>조회</th>
				<td><c:out value="${mediaVO.hitCnt }" /></td>
				<th>최종수정일시</th>
				<td><c:out value="${mediaVO.modDttm }" /></td>
				<th>최종수정자</th>
			...
			<tr>
				...
				<th>기사작성자</th>
				<td>
					<form:input path="writer" maxlength="50"/>
				</td>
			</tr>
			<tr>
				<th>기사원본경로</th>
				<td colspan="5">
					<form:input path="url" maxlength="500" cssStyle="width:90%"/>
				</td>
			</tr>
			<tr>
				<th>제목</th>
...
<sec:authorize access="hasRole('023007001Y')">
	...
	<c:if test="${mediaVO.mdaIdx ne '' && mediaVO.mdaIdx ne null}">
