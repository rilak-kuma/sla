...
jQuery(function($){
	...
	$("#btnRegFm").on('click', function(){
		location.href="/information/notice/noticeInfo.do";
	});
	...
	$("#btnExcelDown").on("click",function(){
		$('form[name=searchFm]')
		 .attr({
			 target: '_blank',
			 action: '<c:url value="/information/notice/noticeListExcel.do" />'
		 })
		 .submit();
...
<div class="subcon_area">
	...
	<div class="stit mt_20 mb_10">
		<strong>센터공지<span class="num">( <c:out value="${paginationInfo.totalRecordCount}" /> )</span></strong>
		<div class="fr">
		<sec:authorize access="hasRole('023007001Y')">
			<button id="btnRegFm" class="btnBasic_blue">공지등록</button>
		</sec:authorize>
...
<%-- 			<c:choose> --%>
<%-- 				<c:when test="${ntcList.topYn eq 'Y' && ntcList.useYn eq 'Y'}"> --%>
					<tr onclick="fn_ntcInfo(${ntcList.ntcIdx })" data-top-yn='${ntcList.topYn }' >
<%-- 				</c:when> --%>
<%-- 				<c:otherwise> --%>
<%-- 					<tr onclick="fn_ntcInfo(${ntcList.ntcIdx })" > --%>
<%-- 				</c:otherwise> --%>
<%-- 			</c:choose> --%>
					<td><c:out value="${ntcList.ntcIdx }" /></td>
					<td><c:out value="${ntcList.titl }" /></td>
					<td><c:out value="${ntcList.creDttm }" /></td>
					<td><c:out value="${ntcList.mngrMbrNm }" />/<c:out value="${ntcList.mngrMbrDpt }" /></td>
...
<form name="infoFm" method="post" action="<c:url value='/information/notice/noticeInfo.do'/>">
	<input type="hidden" id="ntcIdx" name="ntcIdx" />
</form>